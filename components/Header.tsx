...
export default function Header() {
  ...
  return (
    ...
    >
      <nav className="flex items-center justify-between px-8 py-4 max-w-[80rem] w-full text-orange-500 font-bold">
        <Link href="/" className="">
          <Image
            src={Logo}
        ...
        <DesktopNav />
        <Link href="tel:0628142131">
          <motion.button
            whileHover={{
              color: "hsl(var(--muted))",
              backgroundColor: "hsl(var(--primary))",
            }}
            className=" items-center justify-center rounded-full text-orange-500 border-orange-500 border-2 text-sm md:text-lg py-1 px-2 md:py-2 md:px-4 transition-colors flex gap-2"
          >
            <PhoneIcon />
            <p className="">062 814 2131</p>
          </motion.button>
        </Link>
        <MobileMenu />
      </nav>
